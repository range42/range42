...
class Config:
    ...
    INFRASTRUCTURE_PROXMOX_PASSWORD = _getenv("INFRASTRUCTURE_PROXMOX_PASSWORD")

    DEPLOYER_CLI_CONFIG_SSH_NAME = f"range42.{INFRASTRUCTURE_CODENAME}.deployer-cli"

    SSH_CLIENT__DST_CONFIG_DIR = str(Path().home().joinpath(".ssh"))
    SSH_CLIENT__DST_CONFIG_FILE__DEFAULT = str(
        Path(SSH_CLIENT__DST_CONFIG_DIR).joinpath("config")
    )

    SSH_CLIENT__DST_CONFIG_RANGE42_DIR = str(
        Path(SSH_CLIENT__DST_CONFIG_DIR).joinpath(f"range42-{INFRASTRUCTURE_CODENAME}")
    )
    SSH_CLIENT__DST_CONFIG_FILE__RANGE42_DEPLOYER_CLI = str(
        Path(SSH_CLIENT__DST_CONFIG_RANGE42_DIR).joinpath(
            f"config_range42-{INFRASTRUCTURE_CODENAME}"
        )
    )
    SSH_CLIENT__SSH_KEYS_RANGE42_DIR = str(
        Path(SSH_CLIENT__DST_CONFIG_RANGE42_DIR).joinpath("keys")
    )
    SSH_CLIENT__SSH_KEYS_RANGE42_FILE__DEPLOYER_CLI = str(
        Path(SSH_CLIENT__SSH_KEYS_RANGE42_DIR).joinpath(DEPLOYER_CLI_CONFIG_SSH_NAME)
    )

    DEPLOYER_CLI_CONFIG_USER = _getenv("DEPLOYER_CLI_CONFIG_USER")
    DEPLOYER_CLI_CONFIG_PASSWORD = _getenv("DEPLOYER_CLI_CONFIG_PASSWORD", None)
    DEPLOYER_CLI_CONFIG_IP = _getenv("DEPLOYER_CLI_CONFIG_IP")
    DEPLOYER_CLI_CONFIG_PORT = _getenv("DEPLOYER_CLI_CONFIG_PORT", "22")
    ...
    SSH_KEYS_STUDENT_ADDITIONNAL_DIR = f"{INFRASTRUCTURE__AUTO_GENERATED__SSH_KEYS_DIR_LOCAL}/student_keys/additionnal_students/"

    GENERATE_SSH_KEYS_PASSWORD = _getenv("GENERATE_SSH_KEYS_PASSWORD", True, bool)
    ...
    STUDENT_ADDITIONNAL_KEYS_COUNT = _getenv("STUDENT_ADDITIONNAL_KEYS_COUNT", 5, int)

    ALICE_USER_PASSWORD = _getenv("ALICE_USER_PASSWORD", utils.generate_password())
    BOB_USER_PASSWORD = _getenv("BOB_USER_PASSWORD", utils.generate_password())

    JUMP_ON_PROXMOX = _getenv("JUMP_ON_PROXMOX", True, bool)
    INFRASTRUCTURE_JUMP_HOST = _getenv(
        "INFRASTRUCTURE_JUMP_HOST", INFRASTRUCTURE_PROXMOX_ADDRESS
    )
    INFRASTRUCTURE_JUMP_USER = _getenv("INFRASTRUCTURE_JUMP_USER", "jump_user")
    INFRASTRUCTURE_JUMP_USER_WITH_PAM = f"({INFRASTRUCTURE_JUMP_USER}@pam)"
    INFRASTRUCTURE_JUMP_PORT = _getenv("INFRASTRUCTURE_JUMP_PORT", "22")
    INFRASTRUCTURE_JUMP_PASSWORD = _getenv(
        "INFRASTRUCTURE_JUMP_PASSWORD", utils.generate_password()
    )

    INFRASTRUCTURE_PROXMOX_API_HOST = _getenv(
        "INFRASTRUCTURE_PROXMOX_API_HOST", f"{INFRASTRUCTURE_PROXMOX_ADDRESS}:8006"
    )
    INFRASTRUCTURE_PROXMOX_NODE_NAME = _getenv(
        "INFRASTRUCTURE_PROXMOX_NODE_NAME", INFRASTRUCTURE_CODENAME
    )
    INFRASTRUCTURE_PROXMOX_API_USER = _getenv(
        "INFRASTRUCTURE_PROXMOX_API_USER", "api_user"
    )
    INFRASTRUCTURE_PROXMOX_API_TOKEN_ID = _getenv("INFRASTRUCTURE_PROXMOX_API_TOKEN_ID")
    INFRASTRUCTURE_PROXMOX_API_TOKEN_SECRET = _getenv(
